				...
				<table id="mainbox">
					...
					<?php
						...
						$conn = mysqli_connect("localhost", "root", "", "island_express");
			 			...
			 			$sql = "SELECT id, cityfrom, cityto, time FROM timetable";
			 			$result = $conn-> query($sql);

			 			if ($result-> num_rows > 0){
							while($row = $result -> fetch_assoc()){
								echo "<tr><td>" . "<center>" . $row['id'] . "</center>" . "</td><td>" . "<center>" .  $row['cityfrom'] . "</center>" . "</td><td>" . "<center>" .  $row['cityto'] . "</center>" . "</td><td>" . "<center>" .  $row['time'] . "</center>" . "</td></tr>";
							}
							echo "</table>";
			 			}
			 			else{
